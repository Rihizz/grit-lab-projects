...
func VerifyLogin(username, passwordInput string) (int, error) {
	// open db
	db, err := OpenDb()
	if err != nil {
		log.Println(err)
	...
	if exists, err := UserExists(username); !exists || err != nil {
		log.Println(err)
		return 0, err
	}

	//check if password matches
	rows, err := db.Query("SELECT user_id, email, username, password FROM users WHERE username = ?", username)
	if err != nil {
		return 0, err
	}
	defer rows.Close()
